...
const activeTaskDescription = document.querySelector('.active_task_description');
...
function addDeleteEvent(project, index) {
  ...
  element.addEventListener('click', (e) => {
    ...
    while (document.getElementById(`${activeIndex + 1}-del`) !== null) {
      ...
      // task index
      const nextTaskElement = document.getElementById(`${activeIndex + 1}-task`);
      nextTaskElement.id = `${activeIndex}-task`;
      // check mark index
      const nextCheckElement = document.getElementById(`${activeIndex + 1}-check`);
